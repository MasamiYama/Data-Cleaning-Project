...
# 4. Appropriately label the data set with descriptive variaable names
...
colnames(meanandstd)<-gsub("-mean()", "Mean", colnames(meanandstd), ignore.case = TRUE)
...
colnames(meanandstd)<-gsub("angle", "Angle", colnames(meanandstd))
colnames(meanandstd)<-gsub("gravity", "Gravity", colnames(meanandstd))
